...
<head>
    ...
    <title>Answers Dashboard | Takon</title>
...
<body>
    <div class="g-0 container-fluid d-flex" style="height: 100vh;">
        ...
        <div class="col ps-3 pe-4 py-5">
            <h2>ANSWERS DASHBOARD</h2>
            <p>Manage all users answers in apps, you can see the detail of it and you can delete it.</p>
                ...
                <tbody>
                    @foreach ($answers as $answer)
                        <tr>
                            ...
                            <td class="text-truncate" style="max-width: 600px">{{ $answer->answer }}</td>
                            <td class="text-truncate" style="max-width: 200px">{{ $answer->user->name }}</td>
                            <td>
                                <a href="#" class="btn btn-primary btn-sm" data-bs-toggle="modal"
                                    data-bs-target="#detailModal" data-answer-id="{{ $answer->id }}">Detail</a>
                                <a href="#" class="btn btn-danger btn-sm" data-bs-toggle="modal"
                                    data-bs-target="#deleteModal" data-answer-id="{{ $answer->id }}">Delete</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-body text-center">
                        <h4 class="my-3">Are You Sure To Delete This Answer?</h4>
                    </div>
    ...
    <script>
        ...
        $('.btn[data-bs-target="#detailModal"]').click(function() {
            ...
            $.ajax({
                url: '/dashboard/answer-show/' + answerId,
                method: 'get',
                headers: {
                    'X-CSRF-TOKEN': token
                },
                success: function(response) {
                    ...
                    $('#detailModal .modal-body .answer-modal').text(response.answer);
        ...
        $('.btn[data-bs-target="#deleteModal"]').click(function() {
            var answerId = $(this).data('answer-id');
            var token = $('meta[name="csrf-token"]').attr('content');
            var deleteUrl = '{{ route('answerDestroy', ':answerId') }}';
            deleteUrl = deleteUrl.replace(':answerId', answerId);
            $.ajax({
                url: '/dashboard/answer-show/' + answerId,
                method: 'get',
                headers: {
