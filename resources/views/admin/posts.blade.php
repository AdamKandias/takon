...
<head>
    ...
    <title>Posts Dashboard | Takon</title>
...
<body>
    <div class="g-0 container-fluid d-flex" style="height: 100vh;">
        ...
        <div class="col ps-3 pe-4 py-5">
            ...
            <p>Manage all users posts in apps, you can see the detail of post and delete it.</p>
                ...
                <tbody>
                    @foreach ($posts as $post)
                        <tr>
                            ...
                            <td class="text-truncate" style="max-width: 400px">{{ $post->question }}</td>
                            <td>{{ $post->mapel->mapel }}</td>
                            <td class="text-truncate" style="max-width: 200px">{{ $post->user->name }}</td>
                            <td>
                                <a href="#" class="btn btn-primary btn-sm" data-bs-toggle="modal"
                                    data-bs-target="#detailModal" data-post-id="{{ $post->id }}">Detail</a>
                                <a href="#" class="btn btn-danger btn-sm" data-bs-toggle="modal"
                                    data-bs-target="#deleteModal" data-post-id="{{ $post->id }}">Delete</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="d-flex">
                            ...
                            <p class="ms-1 question-modal"></p>
                        </div>
                        <div class="d-flex">
                            <strong>Mapel:</strong>
                            <p class="ms-1 mapel-modal"></p>
                        </div>
                        <div class="d-flex">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-body text-center">
                        <h4 class="my-3">Are You Sure To Delete This Post?</h4>
                    </div>
    ...
    <script>
        ...
        $('.btn[data-bs-target="#detailModal"]').click(function() {
            ...
            $.ajax({
                url: '/dashboard/post-show/' + postId,
                method: 'get',
                headers: {
                    'X-CSRF-TOKEN': token
                },
                success: function(response) {
                    ...
                    $('#detailModal .modal-body .question-modal').text(response.question);
                    $('#detailModal .modal-body .mapel-modal').text(response.mapel.mapel);
        ...
        $('.btn[data-bs-target="#deleteModal"]').click(function() {
            var postId = $(this).data('post-id');
            var token = $('meta[name="csrf-token"]').attr('content');
            var deleteUrl = '{{ route('postDestroy', ':postId') }}';
